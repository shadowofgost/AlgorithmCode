...
def draw_pictures():
    merge_sort_time_result = [
        0.044800305366516115,
        0.050899600982666014,
        0.08979978561401367,
        0.11115198135375977,
        0.1690195083618164,
    ]
    # merge_sort_time_result =[0.1809403657913208, 0.36830027103424073, 0.5818017005920411, 0.7784999370574951, 0.9898996829986573]
    insert_sort_time_result = [
        0.1289997100830078,
        0.4636999130249023,
        0.9908000946044921,
    ...
    # insert_sort_time_result = [21.732965779304504, 88.47726345062256, 191.0323684692383, 326.29902441501616, 484.08071608543395]
    n_list = [10000, 20000, 30000, 40000, 50000]
    x_list = [i for i in range(len(n_list))]
    merge_k = merge_sort_time_result[0] / (log(2, n_list[0]) * n_list[0])
    insert_k = insert_sort_time_result[0] / (n_list[0]) ** 2
    merge_sort_time_predictions = [
        merge_k * merge_sort_time_result[i] for i in range(len(n_list))
    ]
    insert_sort_time_predictions = [
        insert_k * insert_sort_time_result[i] for i in range(len(n_list))
    ]
    from matplotlib import pyplot as plt

    plt.plot(x_list, merge_sort_time_result, label="MergeResult")
    plt.plot(x_list, merge_sort_time_predictions, label="MergePredictions")
    #plt.plot(x_list, insert_sort_time_result, label="InsertResult")
    #plt.plot(x_list, insert_sort_time_predictions, label="InsertPredictions")
    plt.xlabel("n/10000")
    plt.ylabel("time(s)")
    plt.title("Result")
    plt.legend()
